...
use mix_platform::window::WindowId;
use mix_platform::area::Area;
use mix_platform::pass::{PassId, PassClearColor};
use mix_draw::Cx2d;
use mix_draw::draw_list_2d::DrawList2d;
use mix_draw::quad::DrawQuad;
use mix_draw::math::Vec2;
use crate::widget::{Widget, DrawStep, WidgetRef};
use crate::view::View;
...
impl Widget for Window {
    ...
    fn draw(&mut self, cx: &mut Cx2d) -> DrawStep {
        ...
        // Draw background
        let rect = mix_draw::rect::Rect::new(0.0, 0.0, self.size.x, self.size.y);
        self.draw_bg.draw(cx, self.draw_list.id(), &rect);

        // Create a turtle for the content
        cx.begin_sized_turtle(self.size, mix_draw::layout::Layout::vertical());
